...
describe('PhoneNumberValidator', () => {
  ...
  it('should return invalidNumber if both number and country are truthy, but is not valid', () => {
  ...
  it('should return invalid if both number and country are truthy, but phone number parse throws an error', () => {
    control.patchValue({
      country: { name: 'XX', dialingCode: 0, regionCode: 'XX' },
      number: '5555555555',
    });
    expect(validator(control)).toEqual({ phoneNumber: { invalid: true } });
  });
});
