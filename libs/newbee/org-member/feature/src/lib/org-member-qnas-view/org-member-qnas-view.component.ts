...
/**
 * The smart UI for viewing an org member's qnas.
 */
@Component({
  selector: 'newbee-org-member-qnas-view',
  templateUrl: './org-member-qnas-view.component.html',
})
export class OrgMemberQnasViewComponent implements OnDestroy {
  ...
  /**
   * Whether to look for qnas where the org member is the creator or maintainer.
   */
  ...
  /**
   * Subscribe to the route's org member slug and query params and dispatch the request to get qnas.
   */
  constructor(
   ...
   */
  getQnas(): void {
    this.store.dispatch(OrgMemberActions.getQnas({ role: this._role }));
  }
}
