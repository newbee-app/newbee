...
/**
 * A route guard preventing users from accessing the link unless the user is an org member with the permissions to edit the org.
 *
 * @returns `true` if access is allowed, a `UrlTree` to redirect the user otherwise.
 */
export const canEditOrgGuard: CanActivateFn = (): Observable<
  boolean | UrlTree
> => {
  ...
  return store.select(organizationFeature.selectOrgState).pipe(
    ...
    map(({ selectedOrganization, orgMember }) => {
      if (
        checkRoles(apiRoles.org.update, { orgMember: orgMember?.orgMember })
      ) {
        return true;
